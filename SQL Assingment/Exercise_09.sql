...
(select * from `cran_logs_2015-01-01`where substr(time, 1, 5)<"11:00" and substr(time, 1, 5)>"09:00") as a
group by a.package 
order by 2 desc;
...
SELECT COUNT(*) FROM `cran_logs_2015-01-01` WHERE country IN ('CN' , 'JP', 'SG');
